...
public class BrandCommandExecutor extends RapsheetCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        try {
			if (Bukkit.getPlayer(UUIDFetcher.getUUIDOf(args[1])) != null)
			{
				if(args.length == 2) {
					...
					sender.getServer().getPlayer(UUIDFetcher.getUUIDOf(args[1])).sendMessage(ChatColor.GOLD + "Your brand has been removed!");
					return true;
				}
				int x = 2;
				...
				sender.getServer().getPlayer(UUIDFetcher.getUUIDOf(args[1])).sendMessage(ChatColor.RED + "You have been branded as " + ChatColor.translateAlternateColorCodes('&', s));
				return true;
			} else {
